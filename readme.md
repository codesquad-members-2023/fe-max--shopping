...
- [x] 사이드바 UI
- [x] 사이드바 열기
  - [x] 상단바 '모두'버튼을 누르면 화면 좌측에서 사이드바가 나온다
  - [x] 닫기 버튼 생성
  - [x] 딤처리
  - [x] 애니메이션 처리
- [x] 사이드바 닫기 
  - [x] 닫기 버튼을 누르면 사이드바가 원래자리로
  - [x] 애니메이션 처리
- [x] 사이드바 호버
  - [x] 호버시 배경색과 아이콘 색이 변한다
- [x] 사이드바 기능
  - [x] 모두보기를 누르면 접혀있는 카테고리들이 아래로 펼쳐진다
  - [x] 애니메이션 처리
  - [x] 간단히 보기를 누르면 다시 접힌다
  - [x] '부서별 쇼핑' 하위의 카테고리들을 누르면 우측에서 좌측으로 목록이 이동한다
  - [x] 이동하며 상세 카테고리 내역이 보인다
...
ES Modules 를 활용한 개발을 해본다.

- 이미 개발된 사항에 대한 store부분을 view에서 분리해본다.
  - 즉, store와 view가 섞이지 않도록 구현해본다.

- 모듈간 의존성 줄이기 (낮은 결합도)
  - 프로그램을 하다보면 store와 view, 또는 view간의 의존성이 생길 수 있다.
  - 어떠한 관계든 상관없이 의존성을 줄이기 싶은 부분을 선택해서 자유롭게 의존성을 낮춰보자.

- 자기만의 설계
  - 자기만의 방법으로 구조를 설계하고, 이를 PR에 담아서 보낸다.
...
Controller
- 모델과 view 사이에서 중간 역할을 한다
- 비즈니스 로직과 이벤트 처리역할


> 사용자가 controller(Action)를 통해서 Model을 변화시키면 View가 Update

---

플럭스 디자인 패턴 - 단방향 데이터 흐름
기존 MVC에서 발생하는 문제
기존의 MVC 디자인 패턴은 뷰와 모델 간의 복잡한 관계로 이어지며, 뷰가 다른 모델에서 나올 수 있고 무한 루프가 발생할 수 있다



## ES6 모듈 

- ES6 모듈 내부의 모든 항목은 기본적으로 비공개이며, 엄격 모드에서 실행된다('엄격 사용'이 필요 없음 use strict)
- 모듈은 페이지나 다른 모듈에서 참조되는 횟수에 관계없이 한 번만 구문 분석

- 모듈은 MIME 형식의 애플리케이션/자바스크립트로 제공되어야 한다
- 대부분의 서버는 자동으로 이 작업을 수행하지만 동적으로 생성된 스크립트 또는 .mjs 파일에 주의

- 일반 script 태그는 다른 도메인의 스크립트를 가져올 수 있지만 모듈은 CORS(교차 출처 리소스 공유)를 사용하여 가져온다
  - 따라서 다른 도메인의 모듈은 Access-Control-Allow-Origin과 같은 적절한 HTTP 헤더를 설정해야 한다: *.

- script defer 속성은 문서가 로드되고 구문 분석될 때까지 스크립트 실행을 지연
