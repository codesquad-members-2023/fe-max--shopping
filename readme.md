...
### Week #5
- 미구현 기능과 의도를 짝에게 설명하기
- 짝의 설명을 듣고 코드를 읽으며 흐름과 의도를 파악하기
- 모호하거나 이해가 안 되는 부분은 물어보기
...
- [ ] MVC로 UI 로직과 비즈니스 로직을 분리해서 구현한다.
...
**컨트롤러 constructor에서 모델에서 사용할 함수를 넘겨놓고 뷰에서 이벤트 발생시에 핸들러 함수에서 그 함수들을 호출해서 사용하는 모델의 함수를 호출해주는 방식**
남세가 구현한 코드에서는 서버에서 데이터를 받는 비동기 로직이 없어서 가능한지 모르겠다.
검색 컴포넌트의 경우에는 fetch가 필수적이어서, fetch함수를 미리 모델쪽에 등록해놓고 이벤트 발생할 때 호출해서 사용하는 건 힘들어 보임.
왜냐하면 fetch로 받은 데이터로 render를 실행해줘야하기 때문에 비동기 처리를 해줘야하는데, finally로 실행이 되지 않았다.
일단, 미리 등록하는 콜백함수가 아니라, 그 함수를 호출해서 사용하는 함수에 후속 처리하는 콜백을 넘겨서 비동기 처리를 했다.
constructor에서 render 함수를 분리해야 해서 약간의 찝찝함이 남는다. 

### Week#5
**현재까지 진행상황과 개발하려고 했던 부분**
1. 검색, 히어로, 사이드바 3가지의 큰 요소들은 클래스로 개발했습니다.
   -  검색은 추천 검색어와 최근 검색어(더미 데이터)를 불러오고 열기, 닫기만 되는 상태입니다. 검색 기능을 우선적으로 구현할 생각이었습니다.
   -  사이드바의 경우 '레이아웃 + 메뉴'로 열리고 닫히는 기능만 구현되었습니다. 
   -  BaseElement가 있지만 MVC를 적용하면서 사용빈도가 현격히 낮아져 삭제할 예정이었습니다. 
2. 사이드바를 제외한 히어로, 검색은 MVC 패턴을 적용했습니다.
   -  사이드바에도 MVC로 기능을 구현할 계획이었습니다.

정리하면, 검색바, 사이드바의 미구현 기능을 MVC 패턴 적용해서 우선 구현할 계획이었습니다. 히어로에 적용된 MVC와 다르게 Controller에 비즈니스 로직을 몰아넣는 방식으로 구현하려고 했으니 참고바랍니다.
