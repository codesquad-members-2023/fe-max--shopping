...
export class SearchKeyword {
  ...
  addNode(category, keyword) {
    if (category === 'history') {
      $('.search-info').insertAdjacentHTML('afterbegin', this.history.getTemplate(keyword));
    }
    if (category === 'recommend') {
      $('.search-info').insertAdjacentHTML('beforeend', this.recommend.getTemplate(keyword));
    }
    if (category === 'autocomplete') {
      $('.search-info').insertAdjacentHTML('afterbegin', this.autocomplete.getTemplate(keyword));
    }
  }

  renderKeywords(category) {
    this.category.forEach(keyword => addNode(`${category}`, keyword));
  }
}

class History extends SearchKeyword {
  #history = [];
  #MAX_SIZE_OF_RECENT = 5;
  constructor() {
    super();
    this.template = super.this.historyTemplate;
  }
  getTemplate(keyword) {
    return `
      <li class="search-info__history">
...
class Recommend extends SearchKeyword {
  #recommended = [
...
class Autocomplete extends SearchKeyword {
  #autoKeyword = [
    'a형 독감',
    'abc마트',
    'a매치',
